    ...
    return (
        <div className="p-8 max-w-6xl mx-auto relative">
            <header className="mb-8 flex justify-between items-end">
                ...
                {/* Dashboard Tabs */}
            {(() => {
                const ongoing = data.programs.filter(p => p.enrollmentStatus === 'active');
                const completed = data.programs.filter(p => p.enrollmentStatus !== 'active');

                const renderCard = (prog) => {
                    if (prog.enrollmentStatus !== 'active') {
                        return (
                            <Card key={prog.programId} className="bg-gray-50 opacity-75 hover:opacity-100 transition-opacity">
                                <div className="p-6">
                ...
                const renderSection = (programsList, emptyMsg) => {
                    if (programsList.length === 0) {
                        return (
                            <div className="text-center p-8 bg-gray-50 rounded-lg border border-dashed border-gray-300 mt-4">
                ...
                const tabs = [
                    {
                        label: `All (${data.programs.length})`,
                        content: renderSection(data.programs, "You have no programs.")
                    },
                    {
                        label: `Ongoing (${ongoing.length})`,
                        content: renderSection(ongoing, "You have no ongoing programs.")
                    },
                    {
                        label: `Completed (${completed.length})`,
                        content: renderSection(completed, "You have no previous programs.")
                    }
                ];
